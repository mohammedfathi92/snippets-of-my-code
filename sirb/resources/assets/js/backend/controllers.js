...
ctrl.controller('backendUploaderCtrl', function ($scope, $http, Upload, $timeout, $attrs) {
    ...
    // Start city builder CTRL
    .controller("cityBuilderCtrl", function ($scope, $http) {
        $scope.by_cities_ids;
        $scope.country;
        $scope.dataType = "";
        $scope.countriesList = [];
        ...
        $scope.getCountriesList = function () {
            ...
            $http.get("/" + window.locale + "/" + window.backend_uri + "/countries/ajax/list")
                .success(function (resp) {
        ...
        $scope.getCountryCities = function () {
            $scope.citiesList = [];
            if ($scope.country) {
                $http.get("/" + window.locale + "/" + window.backend_uri + "/countries/" + $scope.country + "/getCitiesList").success(function (resp) {
                    if (resp.success) {
                        console.log('hello');
                        $scope.citiesList = resp.data;
                    }
    ...
    .controller("builderCtrl", function ($scope, $http) {
        ...
        $scope.$watchGroup(['dataType', 'country', 'city', 'category'], function (newValues, oldValues, scope) {
        ...
        $scope.getTopics = function () {
            ...
            if ($scope.dataType) {
                ...
                if ($scope.dataType !== 'articles') {
                    $http.get("/" + window.locale + "/" + window.backend_uri + "/landing-pages/country/" + $scope.country + "/city/" + $scope.city + "/" + $scope.dataType).success(function (resp) {
                        if (resp.success) {
                            $scope.topicsList = resp.data;
                        }
                    });
                } else {

                    $http.get("/" + window.locale + "/" + window.backend_uri + "/landing-pages/country/" + $scope.country + "/category/" + $scope.category + "/articles").success(function (resp) {
                        if (resp.success) {
                            $scope.topicsList = resp.data;
                        }
                    });
        ...
        $scope.getCategories = function () {
            $scope.categoriesList = [];
            if ($scope.country) {
                $http.get("/" + window.locale + "/" + window.backend_uri + "/countries/" + $scope.country + "/getCategoriesList").success(function (resp) {
                    if (resp.success) {
                        $scope.categoriesList = resp.data;
                    }
                });
            }
        }
    ...
    .controller("packageHotelsCtrl", function ($scope, $rootScope, $http) {
        ...
        // initialization function  used in update case
                            ...
                            $http.get("/" + window.locale + "/" + window.backend_uri + "/ajax/hotels/" + hotel_id + "/getRoomsList").success(function (resp) {
                                if (resp.success) {
                                    ...
                                    var item = {
                                        ...
                                        hotel: hotel_id,
                                        room: room,
                                        rooms_count: v.pivot.rooms_count,
                                        days: days,
                                        order: order
                                    };
