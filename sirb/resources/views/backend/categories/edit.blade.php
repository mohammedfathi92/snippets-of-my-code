...
@section('page_header')
    <h1 class="page-title">
        <i class="fa fa-clone"></i> {{trans("categories.backend_page_update_header")}} ({{$data->name}})</h1>
@stop
@section("content")
                            ...
                            <select name="parent" id="parent" class=" form-control select2">
                                ...
                                @if($categories=\Sirb\Category::All())
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" {{old("parent",$data->parent_id)==$category->id?"selected":""}} >{{$category->name}}</option>
                                    @endforeach
                                @endif
                            </select>
