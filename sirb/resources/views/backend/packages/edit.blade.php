...
@section('page_header')
    ...
    <div class="panel pale-default">
        <div class="panel-body"><span
                ...
                : <code>{!! Html::link("packages/{$data->id}/".make_slug($data->name),"/packages/{$data->id}/".make_slug($data->name)) !!}</code></span>
        </div>
    </div>
@stop
@section("content")
                ...
                <div class="panel-body">
                    ...
                    <div class="tab-content">
                        <div role="tabpanel" class="tab-pane active" id="tab-general">
                            ...
                            <div class="form-group {{$errors->has("type")?"has-error":''}}">
                                ...
                                <div class="col-sm-3">
                                    <select name="type" id="type" class="form-control select2">
                                        @foreach(\Sirb\PackageType::all() as $type)
                            ...
                            <div class="form-group {{$errors->has("adults_count")?"has-error":''}}">
                                ...
                                <div class="col-sm-9">
                                    <div class="col-md-3">
                                        <label for="adults_count">{{trans("packages.label_adults_count") }} </label>
                                        <input type="number" name="adults_count" min="1" id="adults_count"
                                               value="{{old("adults_count",$data->adults_count)}}"
                                               class="form-control">
                                        @if ($errors->has('adults_count'))
                                            <span class="help-block">
                                            <strong>{{ $errors->first('adults_count') }}</strong>
                                        </span>
                                        @endif
                                    </div>
                                    <div class="col-md-3">
                                        <label for="childrens_count">{{trans("packages.label_childrens_count")}}</label>
                                        <input type="number" name="childrens_count" id="childrens_count" min="1"
                                               value="{{old("childrens_count",$data->childrens_count)}}"
                                               class="form-control">
                                        @if ($errors->has('childrens_count'))
                                            <span class="help-block">
                                            <strong>{{ $errors->first('childrens_count') }}</strong>
                                        </span>
                                        @endif
                                    </div>
                                    <div class="col-md-3">
                                        <label for="childrens_count">{{trans("packages.label_babies_count")}}</label>

                                        <input type="number" name="babies_count" min="1"
                                               value="{{old("babies_count",$data->babies_count)}}"
                                               class="form-control">
                                        @if ($errors->has('babies_count'))
                                            <span class="help-block">
                                            <strong>{{ $errors->first('babies_count') }}</strong>
                                        </span>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div role="tabpanel" class="tab-pane" id="tab-details">
                            ...
                            <div data-ng-controller="packageHotelsCtrl"
                                 data-ng-init="init('{{$data->country_id}}',{{$data->rooms()->orderBy('pivot_order', 'asc')->get()->toJson()}})">

                                <div class="clearfix"></div>

                                {{--Hotels--}}
                                @include("backend.packages.hotels")
                            ...
                            <div data-ng-controller="packageTransportsCtrl"
                                ...
                                <h3 data-toggle="collapse" data-target="#packageTransports" aria-expanded="false"
                                    aria-controls="packageTransports">{{trans('packages.title_package_transports')}}
                                    <span data-ng-show="transportsLoading" class="btn-group-addon"><i
                                ...
                                <fieldset class="collapse group-list  well" id="packageTransports">
                                    ...
                                    <div class="group-list-item ">
                                        ...
                                        <div class="col-md-4">{{trans("packages.label_transport_name")}}</div>
                                        <div class="col-md-1">{{trans("packages.label_order")}}</div>
                                    ...
                                    <div class="group-list-item " data-ng-repeat="item in items ">
                                        ...
                                        <div class="col-md-1">
                                            <input type="number" name="transports[order][]" value="1" min="1"
                                                   class="form-control"
                            ...
                            <div data-ng-controller="packageFlightsCtrl" data-ng-init="init({{$data->flights}})">
                                <h3 data-toggle="collapse" data-target="#packageFlights" aria-expanded="false"
                                    aria-controls="packageFlights">{{trans('packages.title_package_flights')}} <span
                                            data-ng-show="flightsLoading" class="btn-group-addon"><i
                                                class="fa fa-spinner fa-pulse fa-fw"></i></span></h3>
                                {{--Transports--}}
                                ...
                                <fieldset class="collapse group-list  well" id="packageFlights">
                                    ...
                                    <div class="group-list-item ">
                                        ...
                                        <div class="col-md-2">{{trans("packages.label_transport_name")}}</div>
                                        <div class="col-md-1">{{trans("packages.label_order")}}</div>
                                    ...
                                    <div class="group-list-item " data-ng-repeat="item in items ">
                                        ...
                                        <div class="col-md-1">
                                            <input type="number" name="flights[order][]" value="1" min="1"
                                                   class="form-control"
                                                   data-ng-model="item.order">
                                        </div>
                                        <div class="col-md-1">
                                    ...
                                    <div class="">
                                        @if($data->gallery->count())
                                            @foreach($data->gallery as $photo)
                                                @if($photo && Storage::disk('public')->exists(config('settings.upload_dir')."/".$photo->name))
                                                    <div class="thumbnail col-md-6" id="file-{{$photo->name}}">
                                                        <img src="/files/small/{{$photo->name}}" alt=""
                                                             class="img-thumbnail">
                                                        <a href="javascript:;" class="btn btn-danger"
                                                           ng-click="removeByName('{{$photo->name}}')"
                                                           ng-show="!photo">{{trans("main.btn_delete")}}</a>
                                                    </div>
                                                @endif
                                            @endforeach
