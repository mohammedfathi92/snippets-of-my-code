...
@section('page_header')
    <h1 class="page-title">
        <i class="fa fa-file-code-o"></i> {{trans("pages.backend_landing_page_update_header")}} ({{$data->name}})</h1>

    <div class="panel pale-default">
        <div class="panel-body"><span
                    class="help-block">{{trans("main.prime_link")}}
                : <code>{!! Html::link("my/{$data->slug}","/my/{$data->slug}") !!}</code> </span>
        </div>
    </div>
@stop
@section("content")
                        ...
                        <p>
                            @can('edit pages')
                                <a class="btn btn-primary"
                                   href="{{url("/builder/show/$data->id")}}" target="_blank"><i
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group {{$errors->has("slug")?"has-error":''}}">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="slug" class="form-control" id="slug"
                                   placeholder="" value="{{old("slug",$data->slug)}}">
                            @if ($errors->has('slug'))
                                <span class="help-block">
                    ...
                    <hr>
                       <div class="form-group {{$errors->has("menu_id")?"has-error":''}}">
                            <label for="menu_id"
                                   class="control-label col-sm-3">{{trans("pages.label_menu_id")}}
                                <span class="text-danger">*</span>
                            </label>
                            ...
                            <div class="col-sm-9">
                                <select name="menu_id" id="menu_id" class=" form-control select2"
                                    ...
                                    <option value=""></option>
                                    @if(\Sirb\Menu::all())
                                        @foreach(\Sirb\Menu::all() as $menu)
                                            <option value="{{$menu->id}}" {{old("menu_id",$data->menu_id)==$menu->id?"selected":""}} >{{$menu->title}}</option>
                                        @endforeach
                                    @endif
                                </select>
                    ...
                    {{-- <div class="form-group {{$errors->has("in_menu")?"has-error":''}}">
                        ...
                        <div class="col-md-9">
                            <input type="checkbox" name="in_menu" id="in_menu" class="toggle-checkbox"
                                   placeholder=""
                                   value="1" {{!old("in_menu",$data->in_menu)?:"checked"}}>
                        </div>
