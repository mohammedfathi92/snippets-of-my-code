...
<section class="tech-space-area" id="specs">
  <div class="container">
    <ul class="my_list_tools">
      <li><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#editDyBlockFormModal{{$block->id}}"><i class="fa fa-wrench"></i></button></li>
...
<!-- Dynamic Block Modal -->
<div class="modal fade" id="editDyBlockFormModal{{$block->id}}" role="dialog" data-ng-controller="builderCtrl" data-ng-init="getCountriesList()">
...
@if($type !== 'countries' || $type !== 'country_items' || $type !== 'cities' || $type !== 'city_items' )
        ...
        @include('backend.landing_pages.parts.edit_dy_forms', ['content' => $content, 'block' => $block])
       @else
        @include('backend.landing_pages.parts.edit_city_block_forms', ['content' => $content, 'block' => $block])
      @endif
      </div>
