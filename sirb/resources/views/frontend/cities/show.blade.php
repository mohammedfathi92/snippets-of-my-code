    ...
    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
        ...
        @if($localeCode !=LaravelLocalization::getCurrentLocale())
            <link rel="alternate" hreflang="{{$localeCode}}"
                  href="{{\LaravelLocalization::localizeURL("city/{$city->id}/".make_slug($city->name),$localeCode)}}"/>
        @endif
    @endforeach
        ...
        <div class="col-md-9">
            ...
            <div id="hotel-features" class="tab-container">
                <ul class="tabs">
                    ...
                    @endphp
                    @if($hotels->count() && (int)settings('country_hotels_count'))

                        <li><a href="javascript:;" data-target="#country-hotels"
                               data-toggle="tab">{{trans("main.tab_hotels")}}</a></li>
                    @endif
                    @if($places->count() && (int)settings('country_places_count'))
                ...
                <div class="tab-content">
                    ...
                    @if($hotels->count() && (int)settings('country_hotels_count'))
                        <div class="tab-pane fade" id="country-hotels">
                                    ...
                                    <article class="box">
                                        <figure class="col-sm-4 col-md-3">
                                            <a class="hover-effect"
                                               href="{{\LaravelLocalization::localizeURL("/hotels/{$hotel->id}/".make_slug($hotel->name))}}"
                                               title="">
                                        ...
                                        <div class="details col-xs-12 col-sm-8 col-md-9">
                                            <div>
                                                <div>
                                                    <div class="box-title">
                                                        <h4 class="title">{!! Html::link(\LaravelLocalization::localizeURL("/hotels/$hotel->id/".make_slug($hotel->name)),$hotel->name) !!}</h4>
                                                    </div>
                                                    {{--<div class="amenities">
                                            ...
                                            <div>
                                                ...
                                                <div class="action-section">
                                                    <a href="{{\LaravelLocalization::localizeURL("booking/hotel/$hotel->id/".make_slug($hotel->name))}}?rel=book_now"
                                                       title=""
                            ...
                            <a href="{{\LaravelLocalization::localizeURL("city/$city->id/".make_slug($city->name)."/hotels")}}"
                                                   ...
                                                   class="hover-effect {{--popup-gallery--}}">
                                                    @if($place->photo && Storage::disk('public')->exists(config('settings.upload_dir')."/".$place->photo))
                                                        <img class="lazy"
                                                             ...
                                                             height="160"/></a>
                                                @else
                                                    <img class="lazy" src="https://placehold.it/270x160" alt=""
                                                         width="270"
                                                         height="160"/></a>
                                                @endif
                                            </figure>
                                ...
                                @if($item->articles()->count())
                                    @foreach($item->articles as $topic)
                                        <div class="col-sms-6 col-sm-6 col-md-4">
                                            <article class="box">
                                                <figure>
                                                    <a href="{{\LaravelLocalization::localizeURL("guide/$item->id/topic/{$topic->id}/".make_slug($topic->name))}}"
                                                       class="hover-effect {{--popup-gallery--}}">
                                                    ...
                                                    @else
                                                        <img class="lazy img-responsive"
                                                             src="https://placehold.it/270x160"
                                                             alt=""
                                                             width="270"
                                                             height="160"/></a>
                                                    @endif
                                                ...
                                                <div class="details">
                                                    <h4 class="box-title"><a
                                                                href="{{\LaravelLocalization::localizeURL("guide/$item->id/topic/{$topic->id}/".make_slug($topic->name))}}">{{$topic->name}}</a>
                                                    </h4>
            ...
            <!-- Comments $ reviews-->
            @include('frontend.parts.comments_reviews', ['module' => "cities", 'module_data'=>$city])
                        ...
                        @foreach($cities as $item)
                            <article class="box">
                                <figure>
                                    ...
                                    <a href="{{\LaravelLocalization::localizeURL("city/$item->id/".make_slug($item->name))}}">
                                ...
                                <div class="details">
                                    <h5 class="box-title"><a
                                                href="{{\LaravelLocalization::localizeURL("city/$item->id/".make_slug($item->name))}}">{{$item->name}}</a>
                                    </h5>
                                </div>
                            </article>
                            @if($loop->iteration >6)
                            ...
                            @endif
                        @endforeach
                        <a href="{{\LaravelLocalization::localizeURL("country/{$city->country->id}/".make_slug($city->country->name)."/cities")}}"
