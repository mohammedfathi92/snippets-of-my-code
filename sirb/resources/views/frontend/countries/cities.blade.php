    ...
    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
        ...
        @if($localeCode !=LaravelLocalization::getCurrentLocale())
            <link rel="alternate" hreflang="{{$localeCode}}"
                  href="{{\LaravelLocalization::localizeURL("country/{$country->id}/".make_slug($country->name)."/cities",$localeCode)}}"/>
        @endif
    @endforeach
                        ...
                        <div class="row image-box hotel listing-style1">
                            @foreach($cities as $city)
                                <div class="col-sm-6 col-md-4">
                                    <article class="box">
                                        <figure>
                                            <a href="{{\LaravelLocalization::localizeURL("$locale/city/ajax/{$city->id}/gallery")}}"
                                     ...
                                     <div class="details">
                                            ...
                                            <h4 class="box-title"><a
                                                        href="{{\LaravelLocalization::localizeURL("city/$city->id/".make_slug($city->name))}}">{{$city->name}}</a>
                                                <small>
                                                    <a href="{{\LaravelLocalization::localizeURL("country/{$country->id}/".make_slug($country->name ))}}">{{$country->name}}</a>
                                                </small>
                                            </h4>
                                            ...
                                            <div class="action">
                                                <a class="button btn-small"
                                                   href="{{\LaravelLocalization::localizeURL("city/$city->id/".make_slug($city->name))}}">{{trans("cities.btn_show")}}</a>
                                            </div>
