    ...
    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
        ...
        @if($localeCode !=LaravelLocalization::getCurrentLocale())
            <link rel="alternate" hreflang="{{$localeCode}}"
                  href="{{\LaravelLocalization::localizeURL("faq",$localeCode)}}"/>
        @endif
    @endforeach
            ...
            <div class="travelo-box filters-container faq-topics">
                ...
                <ul class="triangle filters-option">
                    <li class="{{(!Request::segment(3))?"active":""}}"><a
                                href="{{\LaravelLocalization::localizeURL("faq")}}">{{trans("faq.text_all_questions")}}</a></li>
                    @foreach($categories->get() as $category)
                        <li class="{{($category->slug==Request::segment(3))?"active":""}}"><a
                                    href="{{\LaravelLocalization::localizeURL("faq/{$category->slug}")}}">{{$category->name}}</a></li>
                    @endforeach
