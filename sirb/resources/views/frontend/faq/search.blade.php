    ...
    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
        ...
        @if($localeCode !=LaravelLocalization::getCurrentLocale())
            <link rel="alternate" hreflang="{{$localeCode}}"
                  href="{{\LaravelLocalization::localizeURL("faq/search?q=".Request::get('q'),$localeCode)}}"/>
        @endif
    @endforeach
    @if($questions->nextPageUrl())
        <link rel="next" href="{{$questions->nextPageUrl()}}">
    @endif
    @if($questions->previousPageUrl())
        <link rel="prev" href="{{$questions->previousPageUrl()}}">
    @endif
@endsection
@section("page_title")
    <div class="page-title-container">
        <div class="container">
            ...
            <ul class="breadcrumbs pull-right">
                ...
                <li><a href="{{\LaravelLocalization::localizeURL("/faq")}}">{{trans("faq.link_faq")}}</a></li>
                <li class="active">{{trans("faq.link_search")}}</li>
            </ul>
        </div>
    </div>
@endsection
@section("content")
    <div class="container">
        <div class="col-sm-4 col-md-3">
            ...
            <div class="travelo-box filters-container faq-topics">
                ...
                <ul class="triangle filters-option">
                    <li class=""><a
                                href="{{\LaravelLocalization::localizeURL("faq")}}">{{trans("faq.text_all_questions")}}</a>
                    </li>
                    @foreach($categories->get() as $category)
                        <li class=""><a
                                    href="{{\LaravelLocalization::localizeURL("faq/{$category->slug}")}}">{{$category->name}}</a>
                        </li>
                    @endforeach
        ...
        <div class="col-sm-8 col-md-9">
            <h2>{{trans("faq.title_search_results")}}</h2>
