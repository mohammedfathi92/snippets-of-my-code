...
class HotelsController extends BackendBaseController
{
    ...
    function store(Request $request)
    {
        ...
        if ($hotel->save()) {
           ...
           // $this->dispatch(new AddUrlToSitemap("/hotels/{$hotel->id}/".str_slug($hotel->{"name:en"}), $hotel->updated_at, "0.9"));

            // AddUrlToSitemap::dispatch("hotels/{$hotel->id}/".str_slug($hotel->{"name:en"}), $hotel->updated_at, "0.9");
        }

//        return redirect($this->data['backend_uri'] . "/hotels")->with(['message' => trans("hotels.success_created"), 'alert-type' => 'success']);
        return redirect()->back()->with(['message' => trans("hotels.success_created"), 'alert-type' => 'success']);
    }
...
//        return redirect($this->data['backend_uri'] . "/hotels")->with(['message' => trans("hotels.success_updated"), 'alert-type' => 'success']);
        return redirect()->back()->with(['message' => trans("hotels.success_updated"), 'alert-type' => 'success']);
