<div class="p-course-item">
	<div class="media course-item-content">
		<img src="{{$book->thumbnail}}" alt="course-name">
		<div class="media-body row">
			<div class="row">
				<div>
					<div><a href="{{route('books.show', ['book' => $book->hashed_id])}}"> {{$book->title}}</a></div>
					<p>{!! str_limit(strip_tags($book->content), 200) !!} </p>
				</div>
				...
				@endif
					...
					{{-- <span class="badge badge-danger">{{ $book->price }} @lang('LMS::attributes.main.currency_rs')</span> --}}
				</div>
			</div>
		</div>
	</div>
