	  ...
	  $skippedAnswersCount = $quizLogs->children()->where('status', 0)->count();
...
@endphp
			...
			<div class="quiz-results faild">
				...
				<div class="result-summary">
					...
					<div class="result-field time">
						...
						<span class="value">{{gmdate("H:i:s", $finishTime)}}</span>
					</div>
				</div>
	...
	<div id="preview_quiz_form" style="margin: 5px">
		{!! Form::model($quiz, ['url' => route('quizzes.get_questions', ['quiz_id' => $quiz->hashed_id, 'log_id' => $quizLogs->hashed_id]).'?show_answer=1','method'=>'GET','files'=>true]) !!}

				<a href="javascript:;" class="btn btn-success preview_quiz_btn"  style="color: #fff;" data-url="{{route('quizzes.quizPage', ['quiz' => $quiz->hashed_id, 'course' => $quizCourseId]).'?show_answers=true'}}"> @lang('developnet-lms::labels.spans.preview_answers') </a>
		...
		{!! Form::model($quiz, ['url' => route('quizzes.retakeQuiz', ['quiz_id' => $quiz->hashed_id]),'method'=>'POST','files'=>true]) !!}
              <input type="hidden" name="quiz_retake" value="1">
				<button type="submit" class="btn btn-primary" id="retake_quiz_btn" style="color: #fff;"> اعادة   الاختبار رقم  {{$quizLogsNum}}</button>
