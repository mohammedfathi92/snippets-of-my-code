                        ...
                        @if(user() && user()->subscribed(null, $plan->id))
                            <a href="#"
                               class="btn btn-rounded {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}">
                                @lang('Packages-admin::labels.pricing.current_package')
                            </a>
                            <br/>
                        ...
                        @else
                            <a class="btn btn-rounded {{ $plan->recommended?'btn-white-border':'btn-dark-border' }}"
                               href="{{ url('subscriptions/checkout/'.$plan->hashed_id) }}">
                                @lang('Packages-admin::labels.pricing.subscribe_now')
                            </a>
                        @endif
                    </div>
                </div>
...
@else
    <p class="text-center text-danger"><strong>@lang('Packages-admin::labels.pricing.product_can_not_found')</strong></p>
@endisset