...
@section('content')
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="@if($active_tab=="profile") active @endif">
                        <a href="#profile" data-toggle="tab">
                            @lang('Packages-admin::labels.auth.profile')
                        </a>
                    </li>
                ...
                <div class="tab-content">
                    <div class="tab-pane @if($active_tab=="profile") active @endif" id="profile">
                        ...
                        <div class="nav-tabs-custom">
                            <ul class="nav nav-pills">
                                <li class="active"><a href="#edit_profile" data-toggle="pill"><i
                                                class="fa fa-pencil"></i> @lang('Packages-admin::labels.auth.edit_profile')</a></li>
                                <li>
                                    <a href="#profile_addresses" data-toggle="pill"><i class="fa fa-map-marker"></i>
                                        @lang('Packages-admin::labels.auth.addresses')</a>
                                </li>
                                <li>
                                    <a href="#reset_password" data-toggle="pill"><i class="fa fa-lock"></i>
                                        @lang('Packages-admin::labels.auth.auth_password')</a>
                                </li>
                                <li>
                                    <a href="#notification_preferences" data-toggle="pill"><i class="fa fa-bell-o"></i>
                                        @lang('Packages-admin::labels.auth.notification_preferences')</a>
                                </li>
                            </ul>
                            <div class="tab-content">
                                <div class="tab-pane active" id="edit_profile">
                                    <div class="row">
                                        <div class="col-md-4">
                                            {!! PackagesForm::text('name','User::attributes.user.name',true) !!}
                                            {!! PackagesForm::email('email','User::attributes.user.email',true) !!}
                                            {!! PackagesForm::text('job_title','User::attributes.user.job_title') !!}
                                        </div>
                                        <div id="country-div" class="col-md-4">
                                            {!! PackagesForm::text('phone_country_code','User::attributes.user.phone_country_code',true,null,['id'=>'authy-countries']) !!}
                                            {!! PackagesForm::text('phone_number','User::attributes.user.phone_number',true,null,['id'=>'authy-cellphone']) !!}
                                        </div>
                                        <div class="col-md-4">
                                            <div class="text-center">
                                                ...
                                                <small class="">@lang('Packages-admin::labels.auth.click_pic_update')</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="tab-pane" id="notification_preferences">
                                    @forelse(\PackagesNotification::getUserNotificationTemplates(user()) as $notifications_template)
                                        <div class="row">
                                            <div class="col-md-12">
                                                {!! PackagesForm::checkboxes(
                                    ...
                                    @empty
                                        <h4>@lang('Packages-admin::labels.auth.no_notification')</h4>
                                    @endforelse
                                </div>
                                <div class="tab-pane" id="reset_password">
                                    <div class="row">
                                        <div class="col-md-4">
                                            {!! PackagesForm::password('password','User::attributes.user.password') !!}
                                            {!! PackagesForm::password('password_confirmation','User::attributes.user.password_confirmation') !!}

                                            @if(\TwoFactorAuth::isActive())
                                                {!! PackagesForm::checkbox('two_factor_auth_enabled','User::attributes.user.two_factor_auth_enabled',\TwoFactorAuth::isEnabled($user)) !!}

                                                @if(!empty(\TwoFactorAuth::getSupportedChannels()))
                                                    {!! PackagesForm::radio('channel','User::attributes.user.channel', false,\TwoFactorAuth::getSupportedChannels(),array_get($user->getTwoFactorAuthProviderOptions(),'channel', null)) !!}
                                                @endif
                                            @endif
                                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                {!! PackagesForm::formButtons(trans('Packages-admin::labels.auth.save',['title' => $title_singular]),[],['href'=>url('dashboard')]) !!}
                            </div>
                        </div>
                        {!! Form::close() !!}
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    ...
                    <h4 class="modal-title"><strong>@lang('Packages-admin::labels.auth.change_image')</strong></h4>
                </div>
    ...
    @lang('Packages-admin::labels.auth.browse_files')
                    ...
                    <button type="button" class="btn btn-default" data-dismiss="modal">@lang('Packages-admin::labels.auth.close')</button>
                </div>
            </div>
        </div>
    </div>
