    ...
    <div class="container padding-bottom-3x mb-1 mt-30">
        @if (count(ShoppingCart::getItems()) > 0)
            <div class="table-responsive shopping-cart">
                <table class="table">
                    <thead>
                    <tr>
                        <th>@lang('Packages-ecommerce-basic::labels.template.cart.product')</th>
                        <th class="text-center" width="200">@lang('Packages-ecommerce-basic::labels.template.cart.quantity')</th>
                        <th class="text-center">@lang('Packages-ecommerce-basic::labels.template.cart.price')</th>
                        <th class="text-center">
                            <a class="btn btn-sm btn-outline-danger" href="{{ url('cart/empty') }}"
                               title="Delete" data-action="post" data-page_action="site_reload">
                                @lang('Packages-ecommerce-basic::labels.template.cart.clear_cart')
                            </a>
                        </th>
                    </tr>
                    </thead>
                    ...
                    @php
                    $item_cats_ids = $item->id->product->categories()->pluck('id')->toArray();
                    $categories_ids = array_merge($categories_ids,$item_cats_ids)
                    @endphp
            ...
            <div class="shopping-cart-footer">
                <div class="column">
                    <div class="column"><a class="btn btn-outline-secondary" href="{{ url('shop') }}"><i
                                    class="icon-arrow-left"></i>&nbsp; @lang('Packages-ecommerce-basic::labels.template.cart.back_shopping')</a></div>
                </div>
                <div class="column text-lg">@lang('Packages-ecommerce-basic::labels.template.cart.subtotal')
                    <span class="text-medium" id="total">
                        {{ ShoppingCart::subTotal() }}
                    </span>
                    <a class="btn btn-success" href="{{ url('checkout') }}">@lang('Packages-ecommerce-basic::labels.template.cart.checkout')</a>
                </div>
            </div>
            {{--<div class="shopping-cart-footer">--}}
            {{--</div>--}}
        @else
            <div class="text-center">
                <h3>@lang('Packages-ecommerce-basic::labels.template.cart.have_no_item')</h3>
                <a href="{{ url('shop') }}" class="btn btn-primary btn">
                    <i class="fa fa-shopping-cart fa-fw"></i>@lang('Packages-ecommerce-basic::labels.template.cart.continue_shopping')
                </a>
            </div>
        @endif

        @include('partials.featured_products',['title'=>trans('Packages-ecommerce-basic::labels.template.cart.title')])
    </div>
@stop
