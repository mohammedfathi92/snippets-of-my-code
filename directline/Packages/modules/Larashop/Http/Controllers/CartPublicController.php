...
use Packages\Modules\Larashop\Http\Requests\AddToCartRequest;
...
class CartPublicController extends PublicBaseController
{
        ...
        try {
            ...
            if ($action == "increaseQuantity" && $itemhash) {
                ...
                if (($quantity > $sku->allowed_quantity) && ($sku->allowed_quantity > 0)) {
                    ...
                    $message = ['level' => 'warning', 'message' => trans('Larashop::exception.cart.item_limited_per_order',['quantity' => $sku->allowed_quantity])];
                    ...
                    $message = ['level' => 'success', 'message' => trans(trans('Larashop::labels.cart.item_has_been_update'))];
                ...
                if (!$cartItem) {
                    $action = "removeItem";
                    $message = ['level' => 'success', 'message' => trans(trans('Larashop::labels.cart.item_has_been_delete'))];
    ...
    public function emptyCart()
    {
        ...
        $message = ['level' => 'success', 'message' => trans(trans('Larashop::labels.cart.cart_empty'))];
        return response()->json($message);
    }
    ...
    public function addToCart(AddToCartRequest $request, Product $product, SKU $sku)
    {
        ...
        try {
            ...
            $message = ['level' => 'success', 'message' => trans(trans('Larashop::labels.cart.product_has_been_add')),
                'action_buttons' => [trans('Larashop::labels.checkout.cart_detail') => url('cart'), trans('Larashop::labels.cart.proceed_checkout') => url('checkout')]
            ];
            $data['total'] = \ShoppingCart::total();
