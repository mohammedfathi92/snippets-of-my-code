    ...
    <div class="row">
        <div class="col-md-8">
            @component('components.box')
                @slot('box_title')
                    @lang('Larashop::labels.order.order_detail')
                @endslot
                <div class="table-responsive">
                    <table class="table">
                        <thead>
                        <tr>
                            <th>@lang('Larashop::attributes.order.id')</th>
                            <th>@lang('Larashop::attributes.order.amount')</th>
                            <th>@lang('Packages::attributes.status')</th>
                            <th>@lang('Larashop::attributes.order.user_id')</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                ...
                @if($downloads = \OrderManager::getOrderDownloadable($order))
                    <h3>@lang('Larashop::labels.order.download_able')</h3>
                    <div class="table-responsive">
                ...
                @if(count($order->posts))
                    <h3>@lang('Larashop::labels.order.private_access')</h3>
                    <ul>
                        @foreach($order->posts as $post)
                            <li>
                                {!! PackagesForm::link(url($post->slug),trans('Larashop::labels.order.magic',['title' => $post->title]),['class'=>'','target'=>'_blank']) !!}
                            </li>
                        @endforeach
                    </ul>
                @endif
        ...
        <div class="col-md-4">
            <div class="row">
                <div class="col-md-12">
                    @component('components.box')
                        @slot('box_title')
                            @lang('Larashop::labels.order.billing_add')
                        @endslot
                        <div class="table-responsive">
                            <table class="table">
                                <tbody>
                                @isset($order->billing['billing_address'])
                                    <tr>
                                        <th>@lang('Larashop::attributes.order.address_one')</th>
                                        <td>{{ $order->billing['billing_address']['address_1'] }}</td>
                                    </tr>
                                    <tr>
                                        <th>@lang('Larashop::attributes.order.address_two')</th>
                                        <td>{{ $order->billing['billing_address']['address_2'] ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                        <th>@lang('Larashop::attributes.order.city')</th>
                                        <td>{{ $order->billing['billing_address']['city'] }}</td>
                                    </tr>
                                    <tr>
                                        <th>@lang('Larashop::attributes.order.state')</th>
                                        <td>{{ $order->billing['billing_address']['state'] }}</td>
                                    </tr>
                                    <tr>
                                        <th>@lang('Larashop::attributes.order.zip')</th>
                                        <td>{{ $order->billing['billing_address']['zip'] }}</td>
                                    </tr>
                                    <tr>
                                        <th>@lang('Larashop::attributes.order.country')</th>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tbody>
                                <tr>
                                    <th>@lang('Larashop::attributes.order.method')</th>
                                    <td>{{ $order->billing['gateway'] ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th>@lang('Larashop::attributes.order.payment_status')</th>
            ...
            @if($order->shipping)
                <div class="row">
                    <div class="col-md-12">
                        @component('components.box')
                            @slot('box_title')
                                @lang('Larashop::labels.order.shipping_details')
                            @endslot
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    @if(isset($order->shipping['tracking_number']) && !empty($order->shipping['tracking_number']))
                                        <tr>
                                            <th>@lang('Larashop::labels.order.tracking_num')</th>
                                            <td>
                                    ...
                                    @if(isset($order->shipping['label_url']) && !empty($order->shipping['label_url']))
                                        <tr>
                                            <th>@lang('Larashop::labels.order.tracking_label')</th>
                                            <td>
                                                <a target="_blank"
                                                   href="{{ $order->shipping['label_url'] }}">
                                                    @lang('Larashop::labels.order.click_here')
                                                </a>
                                            </td>
                                        </tr>
                                    @endif
