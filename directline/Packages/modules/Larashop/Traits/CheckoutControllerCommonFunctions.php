...
use Packages\Modules\Larashop\Classes\Coupons\Advanced;
use Packages\Modules\Larashop\Http\Requests\CheckoutRequest;
...
trait CheckoutControllerCommonFunctions
{
        ...
        try {
            switch ($step) {
                case 'cart-details':
                    ...
                    return view('Larashop::checkout.partials.cart_items')->with(compact('coupon_code'))->render();
                case 'billing-shipping-address':
                    ...
                    return view('Larashop::checkout.partials.address')->with(compact('shipping_address', 'enable_shipping', 'billing_address'))->render();
                case 'select-payment':
                    $gateway = null;
                    ...
                    if (\ShoppingCart::getAttribute('order_id')) {
                        ...
                        $order = Order::create([
                            'amount' => \ShoppingCart::total(false),
                            'currency' => \Payments::session_currency(),
                            'order_number' => \Larashop::createOrderNumber(),
                            'billing' => $billing,
                            'status' => 'pending',
                            'user_id' => $user->id,
                        ]);
                    ...
                    if (!$gateway_name) {
                        ...
                        foreach ($available_gateways as $gateway_key => $available_gateway) {
                            $ecommerce = new Larashop($gateway_key);
                    ...
                    if ($gateway_name) {
                        $ecommerce = new Larashop($gateway_name);
                        $gateway = $ecommerce->gateway;
                    }

                    return view('Larashop::checkout.partials.payment')->with(compact('gateway', 'available_gateways', 'order'))->render();
                    break;
                case
                'shipping-method':
                    ...
                    return view('Larashop::checkout.partials.shipping_methods')->with(['shipping_methods' => $shipping_methods])->render();
                    break;
                case 'order-review':
                    $order = Order::find(\ShoppingCart::getAttribute('order_id'));
                    return view('Larashop::checkout.partials.order_review')->with(['order' => $order])->render();
                    break;
                default:
                    return false;
            }
        ...
        try {
            if ($step == "cart-details") {
                ...
                if ($request->input('coupon_code')) {
                    ...
                    if (!$coupon) {
                        throw new \Exception(trans('Larashop::exception.checkout.invalid_coupon'));
                    }
    ...
    public function gatewayPaymentToken($gateway, Order $order, User $user)
    {
        ...
        try {
            $ecommerce = new Larashop($gateway);
    ...
    {
        $Larashop = new Larashop($checkoutDetails['gateway']);

        $Larashop->payOrder($order, $user, $checkoutDetails);
    }
