...
use Packages\Modules\Larashop\Exceptions\CouponException;
...
trait CouponTrait
{
    ...
    public function checkMinAmount($minAmount, $throwErrors = true)
    {
        ...
        if ($shoppingCart->subTotal(false, false, false) >= $minAmount) {
            ...
            if ($throwErrors) {
                throw new CouponException(trans('Larashop::exception.coupon.must_least_total',['amount' =>  $shoppingCart->formatMoney($minAmount)]));
            } else {
                return false;
            }
        }
    ...
    public function maxDiscount($maxDiscount, $discount, $throwErrors = true)
    {
        ...
        if ($maxDiscount == 0 || $maxDiscount > $discount) {
            ...
            if ($throwErrors) {
                throw new CouponException(trans('Larashop::exception.coupon.max_discount_amount',['amount' =>  $shoppingCart->formatMoney($maxDiscount)]));
            } else {
                return $maxDiscount;
            }
        }
    ...
    public function checkValidTimes(Carbon $startDate, Carbon $endDate, $throwErrors = true)
    {
        if (Carbon::now()->between($startDate, $endDate)) {
            ...
            if ($throwErrors) {
                throw new CouponException(trans('Larashop::exception.coupon.coupon_not_available'));
            } else {
                return false;
            }
        }
    ...
    public function setDiscountOnItem(CartItem $item, $discountAmount)
    {
        if (!is_numeric($discountAmount)) {
            throw new InvalidPrice(trans('Larashop::exception.coupon.must_use_discount_amount'));
        }
