...
use Packages\Foundation\Search\Indexable;
...
use Packages\Modules\Larashop\Facades\Larashop;
use Packages\Modules\Larashop\Traits\DownloadableModel;
use Packages\Modules\Payment\Models\TaxClass;
use Packages\Modules\Payment\Traits\GatewayStatusTrait;
...
class Product extends BaseModel implements HasMedia
{
    ...
    use HasTranslations, Indexable, Sluggable, PresentableTrait, LogsActivity, HasMediaTrait, GatewayStatusTrait, DownloadableModel, ReviewRateableTrait;
    public $translatable = ['name', 'description','caption'];
    ...
    public function getPriceAttribute()
    {
        ...
        if ($type === 'variable' && $price != '-') {
            $price = '<small style="font-size: 9px">' . trans('Larashop::attributes.product.starts_at') . ' </small>' . $price;
        }
    ...
    public function renderProductOptions($type = null, $sku = null, $attributes = [])
    {
        ...
        foreach ($fields as $field) {
            $input .= Larashop::renderAttribute($field, $sku, $attributes);
        }
