...
use Carbon\Carbon;
use Packages\Foundation\Facades\PackagesForm;
...
class Larashop
{
    ...
    function __construct($gateway_key = null, array $params = [])
    {
        if ($gateway_key) {
            ...
            if (!$config) {
                throw new \Exception(trans('Larashop::exception.misc.invalid_gateway'));
            }
    ...
    public function createProduct(Product $product)
    {
        ...
        if ($prod_integration_id) {
            $message = trans('Larashop::exception.misc.product_code_exist', ['arg' => $prod_integration_id]);
        ...
        if ($response->isSuccessful()) {
            ...
            $message = trans('Larashop::exception.misc.create_gateway', ['message' => $response->getMessage()]);
    ...
    public function updateProduct(Product $product)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            $message = trans('Larashop::exception.misc.update_gateway', ['message' => $response->getMessage()]);
    ...
    public function deleteProduct(Product $product, $extra_params = [])
    {
        ...
        if ($response->isSuccessful()) {
            ...
            throw new \Exception(trans('Larashop::exception.misc.delete_product', ['message' => $response->getMessage()]));
        }
    }
    ...
    public function createSKU(SKU $sku)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            // Create SKU failed
            $message = trans('Larashop::exception.misc.create_gateway_sku', ['message' => $response->getMessage()]);
    ...
    public function updateSKU(SKU $sku)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            // update SKU failed
            $message = trans('Larashop::exception.misc.update_gateway_sku', ['message' => $response->getMessage()]);
    ...
    public function deleteSKU(SKU $sku, $extra_params = [])
    {
        ...
        if ($response->isSuccessful()) {
            ...
            throw new \Exception(trans('Larashop::exception.misc.delete_sku', ['message' => $response->getMessage()]));
        }
    }
    ...
    public function createOrder(User $user, $cartItems, $shipping_rate)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            // Create Order failed
            $message = trans('Larashop::exception.misc.create_order_failed', ['message' => $response->getMessage()]);
    ...
    public function updateOrder(Order $order, User $user, $cart_items)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            if (!$order) {
                throw new \Exception(trans('Larashop::exception.misc.invalid_order_code', ['data' => $data['id']]));
            }
            $order->items()->delete();
            ...
            // update Order failed
            $message = trans('Larashop::exception.misc.update_gateway_order_failed', ['message' => $response->getMessage()]);
    ...
    public function payOrder(Order $order, User $user, $checkoutDetails)
    {
        if (isset($order->billing['payment_status']) && ($order->billing['payment_status'] == 'paid')) {
            throw new \Exception(trans('Larashop::exception.misc.order_already_paid'));
        }
       ...
       return $categories;
        ...
        if ($parentsOnly) {
            ...
            foreach ($categories as $category) {
                $categoriesResult = $category;//$this->appendCategory($categoriesResult, $category);
            }
    ...
    public function createPaymentToken(Order $order, User $user)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            throw new \Exception(trans('Larashop::exception.misc.gateway_create_payment', ['data' => $response->getDataText()]));
        }
    }
    ...
    public function renderAttribute($field, $sku = null, $attributes = [])
    {
        ...
        switch ($field->type) {
            case 'label':
                ...
                $input = PackagesForm::{$field->type}('options[' . $field->id . ']', $field->label, $attributes);
                break;
            case 'number':
            case 'date':
            case 'text':
            case 'textarea':
                $input = PackagesForm::{$field->type}('options[' . $field->id . ']', $field->label, $asFilter ? false : $field->required, $value, $attributes);
                break;
            case 'checkbox':
                $input = PackagesForm::{$field->type}('options[' . $field->id . ']', $field->label, $value, 1, $attributes);
                break;
            case 'radio':
                $input = PackagesForm::{$field->type}('options[' . $field->id . ']', $field->label, $asFilter ? false : $field->required, $field->options->pluck('option_display', 'id')->toArray(), $value, $attributes);
                break;
            case 'select':
                $input = PackagesForm::{$field->type}('options[' . $field->id . ']', $field->label, $field->options->pluck('option_display', 'id')->toArray(), $asFilter ? false : $field->required, $value, $attributes, 'select2');
                break;
            case 'multi_values':
                $attributes = array_merge(['class' => 'select2-normal', 'multiple' => true], $attributes);
                $input = PackagesForm::select('options[' . $field->id . '][]', $field->label, $field->options->pluck('option_display', 'id')->toArray(), $asFilter ? false : $field->required, $value, $attributes, 'select2');
                break;
        }
