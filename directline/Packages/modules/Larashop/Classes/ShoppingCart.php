...
use Packages\Modules\Larashop\Contracts\ShoppingCartContract;
use Packages\Modules\Larashop\Exceptions\ModelNotFound;
...
class ShoppingCart implements ShoppingCartContract
{
    ...
    )
    {
        if (!empty(config('shoppingcart.item_model'))) {
            ...
            $itemID = $itemModel[$bindings[\Packages\Modules\Larashop\Classes\CartItem::ITEM_ID]];

            if (is_int($name)) {
                $qty = $name;
            }

            $name = $itemModel[$bindings[\Packages\Modules\Larashop\Classes\CartItem::ITEM_NAME]];
            $price = $itemModel[$bindings[\Packages\Modules\Larashop\Classes\CartItem::ITEM_PRICE]];

            //$options['model'] = $itemModel;

            // $options = array_merge($options,
            //    $this->getItemModelOptions($itemModel, $bindings[\Packages\Modules\Larashop\Classes\CartItem::ITEM_OPTIONS]));

            $taxable = $itemModel[$bindings[\Packages\Modules\Larashop\Classes\CartItem::ITEM_TAXABLE]] ? true : false;
        }
     ...
     * @param $value
     *
     * @throws \Packages\Modules\Larashop\Exceptions\InvalidPrice
     * @throws \Packages\Modules\Larashop\Exceptions\InvalidQuantity
     *
     * @return CartItem|null
     */
