...
use Packages\Foundation\Search\Search;
...
class Shop
{
   ...
   public function getRelatedProducts($categories_ids)
    {
        $products = Product::active()->whereHas('categories', function ($query) use ($categories_ids) {
    $query->whereIn('id', $categories_ids);
})->take(4)->get();
...
//        });
    ...
    public function getAttributesForFilters()
    {
        ...
        foreach ($attributes as $attribute) {
            $filters .= LarashopFacade::renderAttribute($attribute, null, ['as_filter' => true]);
        }
