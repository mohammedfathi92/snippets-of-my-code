    ...
    public function validate($throwErrors = false, $user = null)
    {
        ...
        if ($this->settings->users->count() > 0) {
            if ($this->settings->users->contains($user->id)) {
                ...
                if ($throwErrors) {
                    throw new CouponException(trans('Larashop::exception.coupon.not_eligible_use_coupon'));
                } else {
                    return false;
                }
            }
        ...
        if ($this->settings->uses > 0) {
            ...
            if ($usage >= $this->settings->uses) {
                if ($throwErrors) {
                    throw new CouponException(trans('Larashop::exception.coupon.code_reached_maximum'));
                } else {
                    return false;
                }
            }
