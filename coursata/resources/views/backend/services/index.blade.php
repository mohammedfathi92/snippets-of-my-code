...
<div class="row">
  <div class="col-md-6 col-sm-6">
      ...
      <div class="col-md-6 col-md-6">            
       ...
       <a data-toggle="modal" href="#addBasicServicesModal" class="btn btn-primary">{{trans("institutes.add_basic_services_btn")}}</a>
       </div> 
...
@section("content")
    <div class="page-content container-fluid">
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <div class="table-responsive">
                            {!! Form::open(['url'=>"$locale/$backend_uri/institutes/services/delete",'name'=>"tableForm",'id'=>'tableForm']) !!}
                                ...
                                @if($services->count())
                                    @foreach($services as $row)
                                        <tr>
                                            ...
                                            <td class="text-right">
                                                @can('edit institutes')
                                                    <a href="{{url("$locale/$backend_uri/institutes/{$row->institute_id}/services/{$row->id}/edit")}}"
                                                       class="btn btn-default btn-xs" data-toggle="tooltip"
                                                       data-placement="top" title="{{trans("main.tooltip_edit")}}"><i
                                                                class="fa fa-pencil"></i></a>
                                                @endcan
                                                @can('delete institutes')
                                                    <a href="{{url("$locale/$backend_uri/institutes/{$row->institute_id}/services/{$row->id}/delete")}}"
       ...
       <!-- Modal -->
  <div class="modal fade" id="addBasicServicesModal" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">{{trans("institutes.pop_title_basic_services")}}</h4>
        </div>
        <div class="modal-body">
        
           @if(\Corsata\BasicService::all())

           {!! Form::open(['url'=>$locale.'/'.$backend_uri.'/institutes/'.$institute->id.'/services/add-basic-services', 'class'=>'form-horizontal','name'=>'form','novalidate']) !!}

           @foreach(\Corsata\BasicService::all() as $basic_service)


           <div class="form-group {{!$errors->has("basic_services")?:"has-error"}} ">
                                    
                                    <div class="col-sm-9">
                                        <input type="checkbox" name="basic_services[]"
                                               id="basic_services"
                                             value="{{$basic_service->id}}"> {{$basic_service->name}}
                                        @if ($errors->has("basic_services"))
                                            <span class="help-block">
                                                    <strong>{{ $errors->first("basic_services") }}</strong>
                                                </span>
                                        @endif
                                    </div>
                                </div>
                                @endforeach
                                <button type="submit" class="btn btn-success">{{trans("institutes.btn_add_services")}}</button>
                                {!! Form::close() !!}

                                @else

                                <p>No Basic Services Found.... .</p>
