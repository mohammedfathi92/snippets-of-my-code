...
@section('page_header')
    <h1 class="page-title">
        <i class="fa fa-plane"></i> {{trans("transports.backend_page_update_header")}} </h1>
@stop
@section("content")
                                ...
                                <div class="form-group {{!$errors->has("company.$localeCode")?:"has-error"}} ">
                                    ...
                                    <div class="col-sm-9">
                                        <input type="text" name="company[{{$localeCode}}]" class="form-control"
                                               id="company"
                                               placeholder=""
                                               value="{{old("company.$localeCode",$data->{"company_name:$localeCode"})}}">
                    ...
                    <div class="form-group {{$errors->has("stars")?"has-error":''}}">
                        ...
                        <div class="col-sm-4">
                            <select name="type" id="type" class="form-control select2">
                                <option value="flight" {{old("type",$data->type)=='flight'?"selected":""}}>{{trans("transports.transport_type.flight")}}</option>
                                <option value="ship" {{old("type",$data->type)=='ship'?"selected":""}}>{{trans("transports.transport_type.ship")}}</option>
                                <option value="bus" {{old("type",$data->type)=="bus"?"selected":""}}>{{trans("transports.transport_type.bus")}}</option>
                                <option value="car" {{old("type",$data->type)=="car"?"selected":""}}>{{trans("transports.transport_type.car")}}</option>
                            </select>
                            @if ($errors->has('type'))
                                <span class="help-block">
                            ...
                            <div class="">
                                @if($data->photo && Storage::disk('public')->exists(config('settings.upload_dir')."/".$data->photo))
                                    <div class="thumbnail" id="file-{{$data->photo}}">
                                        <img src="{{url("files/small/$data->photo")}}" alt="" class="img-thumbnail">
                                ...
                                @if($data->gallery()->count())
                                    @foreach($data->gallery as $photo)
                                        @if($photo && Storage::disk('public')->exists(config('settings.upload_dir')."/".$photo->name))
                                            <div class="thumbnail" id="file-{{$photo->name}}">
                                                <img src="{{url("files/small/$photo->name")}}" alt=""
                                                     class="img-thumbnail">
                                                <a href="javascript:;" class="btn btn-danger"
