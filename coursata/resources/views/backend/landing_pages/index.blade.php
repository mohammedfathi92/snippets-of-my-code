                        ...
                        <p>
                            @can('create pages')
                                <a class="btn btn-primary"
                                   href="{{url(config("settings.backend_uri")."/builder/create")}}"><i
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <div class="table-responsive">
                            {!! Form::open(['url'=>"$locale/$backend_uri/builder/delete",'name'=>"tableForm",'id'=>'tableForm']) !!}
                            <table id="dataTable" class="table v-middle dataTable">
                                ...
                                @if($data=\Corsata\LandingPage::all())
                                    @foreach($data as $row)
                                        ...
                                        <tr>
                                            ...
                                            @endcan
                                            <td>{{$row->id}}</td>
                                            <td><a href="{{url("top/{$row->slug}")}}"
                                                   target="_blank">{{$row->name}}</a></td>
                                            <td>{!! $row->{"json_code:ar"}?Html::link("/builder/page/$row->id?lang=ar","Edit Page - AR",['class'=>'btn btn-primary']):Html::link("/builder/page/$row->id?lang=ar","Build Page - AR",['class'=>'btn btn-primary']) !!}</td>
                                            <td>{!! $row->{"json_code:en"}?Html::link("/builder/page/$row->id?lang=en","Edit Page - AR",['class'=>'btn btn-primary']):Html::link("/builder/page/$row->id?lang=en","Build Page - AR",['class'=>'btn btn-primary']) !!}</td>       
                                            ...
                                            <td class="text-right">
                                                @can('edit pages')
                                                    <a href="{{url("$locale/$backend_uri/builder/{$row->id}/edit")}}"
                                                       class="btn btn-default btn-xs" data-toggle="tooltip"
                                                       data-placement="top" title="{{trans("main.tooltip_edit")}}"><i
                                                                class="fa fa-pencil"></i></a>
                                                @endcan
                                                @can('delete pages')
                                                    <a href="{{url("$locale/$backend_uri/builder/{$row->id}/delete")}}"
