...
@section('page_header')
    <h1 class="page-title">
        <i class="fa fa-map"></i> {{trans("courses.backend_page_header")}}
        <a href="{{url($locale.'/'.$backend_uri."/institutes/$institute->id/courses/create")}}" class="btn btn-success"><i
                        ...
                        <p>
                            @can('create institutes')
                                <a class="btn btn-primary"
                                   href="{{url($locale.'/'.$backend_uri."/institutes/$institute->id/courses/create")}}"><i
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <div class="table-responsive">
                            {!! Form::open(['url'=>"$locale/$backend_uri/institutes/$institute->id/courses/delete",'name'=>"tableForm",'id'=>'tableForm']) !!}
                            <table id="dataTable" class="table v-middle dataTable">
                                ...
                                <tr>
                                    ...
                                    @endcan
                                    <th>{{trans("courses.id")}}</th>
                                    <th>{{trans("courses.photo")}}</th>
                                    <th>{{trans("courses.name")}}</th>
                                    <th>{{trans("courses.institute")}}</th>
                                    <th>{{trans("courses.status")}}</th>
                                    <th>{{trans("courses.last_update")}}</th>
                                ...
                                @if($data)
                                    @foreach($data as $row)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{Html::link("/courses/{$row->id}-".str_slug($row->{"name:en"}),$row->name)}}</td>
                                            <td>{{$row->institute->name}}</td>
                                            <td>{!! $row->status?'<span class="label label-success">'.trans("courses.status_active").'</span>':'<span class="label label-danger">'.trans("courses.status_inactive").'</span>' !!}</td>
                                            <td>{!! \Carbon\Carbon::instance($row->updated_at)->diffForHumans() !!}</td>
                                            <td class="text-right">
                                                @can('edit institutes')
                                                    <a href="{{url("$locale/$backend_uri/institutes/{$row->institute->id}/courses/$row->id/edit")}}"
                                                       class="btn btn-default btn-xs" data-toggle="tooltip"
                                                       data-placement="top" title="{{trans("main.tooltip_edit")}}"><i
                                                                class="fa fa-pencil"></i></a>
                                                @endcan
                                                @can('delete institutes')
                                                    <a href="{{url("$locale/$backend_uri/institutes/{$row->institute->id}/courses/$row->id/delete")}}"
