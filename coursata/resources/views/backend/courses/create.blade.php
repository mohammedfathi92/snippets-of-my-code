...
@section('page_header')
    <h1 class="page-title">
        <i class="fa fa-h-square"></i> {{trans("courses.backend_page_create_header")}} </h1>
@stop
@section("content")
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group {{$errors->has("avg_students")?"has-error":''}}">
                        ...
                        <div class="col-sm-3">
                            <input type="number" name="avg_students" class="form-control" id="avg_students" min="1"
                                   min="1"
                                   placeholder="" value="{{old("avg_students",1)}}">
                    ...
                    <div class="form-group {{$errors->has("max_students")?"has-error":''}}">
                        ...
                        <div class="col-sm-3">
                            <input type="number" name="max_students" class="form-control" id="max_students" min="1"
                                   min="1"
                                   placeholder="" value="{{old("max_students",1)}}">
                    ...
                    <div class="form-group {{$errors->has("start_day")?"has-error":''}}">
                        ...
                        <div class="col-sm-4">
                            <select name="start_day" id="start_day" class="form-control">
                                <option value="mo" {{old("start_day")=='mo'?"selected":""}}>{{trans("courses.week_days_options.mo")}}</option>
                                <option value="tu" {{old("start_day")=='tu'?"selected":""}}>{{trans("courses.week_days_options.tu")}}</option>
                                <option value="we" {{old("start_day")=='we'?"selected":""}}>{{trans("courses.week_days_options.we")}}</option>
                                <option value="th" {{old("start_day")=='th'?"selected":""}}>{{trans("courses.week_days_options.th")}}</option>
                                <option value="fr" {{old("start_day")=='fr'?"selected":""}}>{{trans("courses.week_days_options.fr")}}</option>
                                <option value="sa" {{old("start_day")=='sa'?"selected":""}}>{{trans("courses.week_days_options.sa")}}</option>
                                <option value="su" {{old("start_day")=='su'?"selected":""}}>{{trans("courses.week_days_options.su")}}</option>
                            </select>
                    ...
                    <div class="form-group {{$errors->has("category")?"has-error":''}}"
                         data-ng-controller="coursesCategoriesQuickAdd">
                        ...
                        <div class="col-md-1">
                            <button type="button" class="btn btn-default" data-toggle="modal" data-target="#myModal"
                                    data-ng-click="add()"><i class="icon icon-list-add"></i></button>
                        </div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">{{trans("categories.modal_title_add")}}
                            <div class="modal-body">
                                <div id="errors">

                                </div>
                                {!! Form::open(['url'=>route("categories.ajax.create"),'class'=>'form-horizontal','id'=>'addCategoryForm','name'=>'addCategoryForm','novalidate']) !!}
                                <div>
                                    <div class="panel panel-primary">
                                        <div class="panel-body">
                                            <!-- Nav tabs -->
                                            <ul class="nav nav-tabs nav-tabs-solid" role="tablist">
                                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                    ...
                                                    <li role="presentation" class="{{$loop->index==0?"active":""}}"><a
                                                                href="#modalLang-{{$localeCode}}"
                                                                aria-controls="modalLang-{{$localeCode}}" role="tab"
                                                                data-toggle="tab">{{ $properties['native']}}</a>
                                                    </li>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" save-modal-form-data
                                        data-modal-id="addCategoryModal">Save changes
                                </button>
                            </div>
                    </div>
                </div>
