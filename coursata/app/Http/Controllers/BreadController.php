...
//    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect(Settings::get('backend_uri') . $dataType->slug)->with(array('message' => 'Successfully Updated ' . $dataType->display_name_singular, 'alert-type' => 'success'));
    }
    ...
    public function getContentBasedOnType($request, $slug, $row)
    {
        ...
        if ($row->type == 'password') {
            ...
            if ($request->hasFile($row->field)) {
                ...
                Storage::disk('public')->put(trim(config('settings.upload_dir'), "/") . "/" . $full_path, (string)$image, 'public');
                ...
                if (isset($options->thumbnails)) {
                    foreach ($options->thumbnails as $thumbnails) {
                        ...
                        Storage::disk('public')->put(trim(config('settings.upload_dir'), "/") . "/" . $path . $filename . '-' . $thumbnails->name . '.' . $file->getClientOriginalExtension(), (string)$image, 'public');
                    }
                }
    ...
    public function store(Request $request)
    {
        ...
        return redirect(Settings::get('backend_uri') . $dataType->slug)->with(array('message' => 'Successfully Added New ' . $dataType->display_name_singular, 'alert-type' => 'success'));
    }
...
public function destroy(Request $request, $id)
    {
        ...
        if ($data->destroy($id)) {
            return redirect(Settings::get('backend_uri')."/" . $dataType->slug)->with(array('message' => 'Successfully Deleted ' . $dataType->display_name_singular, 'alert-type' => 'success'));
        }

        return redirect(Settings::get('backend_uri')."/" . $dataType->display_name_singular)->with(array('message' => 'Sorry it appears there was a problem deleting this ' . $dataType->display_name_singular, 'alert-type' => 'error'));
