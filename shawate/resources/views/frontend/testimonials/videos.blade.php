            ...
            <div class="travelo-box filters-container faq-topics">
                ...
                <ul class="triangle filters-option">
                    <li class="{{(!Request::segment(3))?"active":""}}"><a
                                href="{{url("testimonials/videos")}}">{{trans("testimonials.link_all_destinations")}}</a>
                    </li>
                    @foreach(\App\Country::published()->get() as $c)
                        <li class="{{($c->id==Request::segment(4))?"active":""}}"><a
                                    href="{{url("testimonials/videos/destination/$c->id/".str_slug($c->{"name:en"}))}}">{{trans("testimonials.travelers_to_country",["country"=>$c->name])}}</a>
                        </li>
                    @endforeach
