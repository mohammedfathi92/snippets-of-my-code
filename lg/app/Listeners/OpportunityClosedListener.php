...
class OpportunityClosedListener
{
    /**
     ...
     * @return void
     */
    public function handle(OpportunityClosed $event)
    {
        $user=Auth::user();
        ...
        $notification->message="notifications.info_opportunity_closed";
        ...
        // send email to user after his opportunity closed
            ...
            Mail::send("emails.user_opportunity_closed",["data"=>$event->opportunity,"language"=>$language],function ($m) use ($event,$language){
                $m->from(config("settings.app_email"),trans(config("settings.app_title")));
                $m->to($event->opportunity->user->email,$event->opportunity->user->name)->subject(trans("emails.user_opportunity_closed",[],'messages',$language));
            });
