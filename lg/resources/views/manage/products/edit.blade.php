        ...
        <div class="panel">
            <div class="panel">
                <ol class="breadcrumb">
                    ...
                    <li class="active"> {{$data->name}} ({{trans("products.link_edit")}})</li>
                </ol>
            </div>
            <div class="panel-body container-fluid">
                ...
                <div class="row row-lg">
                                    ...
                                    @foreach(LaravelLocalization::getSupportedLocales() as $locale=> $properties)
                                        <div class="form-group  {{App::isLocale($locale)?"required":""}} {{$errors->has("name.$locale")?'has-error':''}}">
                                            <label class="control-label col-md-2"
                                                   for="name_ar">{{$properties['native']}}</label>
                                            <div class="col-md-10">
                                ...
                                <div class="col-sm-9">
                                    @foreach(LaravelLocalization::getSupportedLocales() as $locale=> $properties)
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-10">
                                                {!! Form::textarea("details[$locale]",old("details.$locale",$data->{"details:$locale"}),['class'=>'form-control','id'=>"description_$locale"]) !!}
                                    ...
                                    <select class="form-control select2" name="category">
                                        @if($categories)
                                            ...
                                            @foreach($categories as $category)
                                                <option value="{{$category->id}}" {{$category->id==old('category',$data->category_id)?"selected":""}}>{{$category->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="col-sm-9">
                                    @if(count($categories))
                                        ...
                                        <select class="form-control select2" name="type">
                                            ...
                                            <option value="b2b" {{old("type",$data->product_type)=='b2b'?"selected":""}} >{{trans("products.option_type_b2b")}}</option>
                                            <option value="b2c" {{old("type",$data->product_type)=='b2c'?"selected":""}} >{{trans("products.option_type_b2c")}}</option>
                            ...
                            <div class="form-group {{$errors->has('promotion')?'has-error':''}}">
                                ...
                                <div class="col-sm-3">
                                    <input type="number" id="promotion" name="promotion"
                                           value="{{old("promotion",$data->promotion)}}" class="form-control"
                                           min="0">
                    ...
                    <div class="col-md-4 col-sm-4  {{$errors->has('photo')?'has-error':''}}"
                        ...
                        @if($data->photo)
                            <div class="thumbnail">
                                <img src="{{url("/images/sm/".$data->photo)}}" alt="">
